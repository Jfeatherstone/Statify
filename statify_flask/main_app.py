...
from .data_process import authenticate, fetchPublicPlaylists, mergePlaylistTracks, computePCA, computeLDA, interpretDecomposedCoordinates, decompositionHull
...
def home():
    ...
    if request.method == 'POST':
        ...
        if username:
            ...
            playlists = None
            try:
                playlists = fetchPublicPlaylists(username)
            ...
            phaseSpace, identities, featureNames = mergePlaylistTracks(playlists.iloc[activePlaylistIndices]["uri"])

            # DEBUG: just start with 2d PCA
            if method == 'pca':
                decompCoords, pcaComponents = computePCA(phaseSpace, reducedDims)

                # Interpret the decomposed axes
                interpretedLabels = interpretDecomposedCoordinates(pcaComponents, featureNames)

            elif method == 'lda':  
                decompCoords = computeLDA(phaseSpace, identities, reducedDims)
            ...
            # Generate the figure
            fig = decompositionHull(decompCoords, identities, idToName, 0.05)
